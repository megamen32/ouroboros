...
async function enqueueSecretaryTask(m, selfId) {
  ...
  const description = [
    "Входящее личное сообщение Telegram для персонального секретаря. Primary working language: Russian.",
    "",
    `Отправитель: user id ${m.fromId}; диалог: ${m.peer.id}; message id: ${m.id}; время: ${dateIso}.`,
    `Текст сообщения: «${text}»`,
    "",
    "Следуй personal_information_secretary.md как единственному source of truth для secretary workflow.",
    "Старые Telegram-specific mandate/curfew/digest правила считаются deprecated и не должны переопределять текущую миссию.",
    "Используй минимально необходимый AFFiNE/UserIO контекст, сохраняй provenance и не выдумывай факты.",
    "Если сообщение содержит поручение, решение, обязательство или полезный новый факт — обнови соответствующие AFFiNE/Todo артефакты без дублей.",
    "Если полезен ответ, подготовь или обнови UserIO draft. Никогда не отправляй Telegram-сообщение автоматически из этого task.",
    "Не уходи в инфраструктурные self-checks, shell/VCS или переписывание MCP-конфигурации, если само сообщение этого явно не требует.",
  ].join("\n");
